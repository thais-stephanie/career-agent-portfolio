...
```
<json.path>  <raw value exactly as the ATS stored it>
```
...
offices.0.location     Remote (EMEA)
```
...
`source_field` and `source_value` **byte-for-byte** as given. They are checked
against the archived payload. Do not reformat, re-order JSON keys, or trim.
