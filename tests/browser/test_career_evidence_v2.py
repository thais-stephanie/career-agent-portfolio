...
def summary(page: Chrome) -> str:
    return str(page.evaluate("document.querySelector('#page-manage [data-summary]').textContent"))


def rows(page: Chrome) -> list[str]:
    return list(
        page.evaluate(
            "[...document.querySelectorAll('#page-manage .cvr__rows[aria-label] .cvr__row')]"
...
def test_a_long_cv_is_fast_and_grouped(page: Chrome, pristine_server: str) -> None:
    ...
    assert page.evaluate("document.querySelectorAll('#page-manage .ev__card').length") == 0
    started = time.perf_counter()
    page.evaluate("document.querySelector('#page-manage .cvr__row button').click()")
    page.wait_for("document.querySelectorAll('#page-manage .ev__card').length === 14")
...
def test_next_item_walks_the_read_and_nothing_is_confirmed_in_bulk(
    ...
    page.wait_for("document.querySelectorAll('#page-manage .ev__card').length > 0")
    heading = str(
        page.evaluate("document.querySelector('#page-manage .cvr__group h3').textContent")
    )
    ...
    buttons = page.evaluate(
        "[...document.querySelectorAll('#page-manage button')].map(b => b.textContent)"
    )
    ...
    page.wait_for("document.querySelectorAll('#page-manage .ev__decision').length === 1")
    ...
    page.wait_for(
        "document.querySelector('#page-manage .cvr__group h3').textContent"
...
def test_archive_and_restore_empty_and_refill_every_counter(
    ...
    page.evaluate("document.querySelector('#page-manage [data-action=\"archive\"]').click()")
    page.wait_for("document.querySelector('#page-manage .cvr__archived') !== null")
    ...
    assert not page.evaluate(
        "Boolean(document.querySelector('#page-manage [data-action=\"next\"]'))"
    )
...
def test_the_wrong_cv_is_deleted_after_saying_exactly_what_goes(
    ...
    open_evidence(page, pristine_server)
    choose_file(page, long_cv(11, 13), "wrong.txt")
    page.evaluate("document.querySelector('#page-manage .cvr [data-action=\"delete\"]').click()")
    page.wait_for("document.querySelector('#page-manage .cvr__confirm') !== null")
...
def test_an_unresolved_experience_is_corrected_not_guessed(
    ...
    page.evaluate(
        "[...document.querySelectorAll('#page-manage .cvr__row')]"
        ".find((r) => r.textContent.includes('Umbrella')).querySelector('button').click()"
    )
    page.wait_for("document.querySelector('#page-manage .cvr__group .cvr__attention') !== null")
    press_button(page, "Correct company, role or dates")
    page.evaluate(
        "(() => { const inputs = document.querySelectorAll('#page-manage .cvr__form input');"
    ...
    assert not page.evaluate(
        "[...document.querySelectorAll('#page-manage .cvr__group > .cvr__attention')].length"
    )
    source = str(page.evaluate("document.querySelector('#page-manage .cvr__source').textContent"))
...
def test_the_review_fits_a_phone_and_every_control_is_named(
    ...
    unnamed = page.evaluate(
        "[...document.querySelectorAll('#page-manage .cvr button, #page-manage .cvr select,"
        " #page-manage .cvr input')].filter((n) => !(n.getAttribute('aria-label')"
...
def test_imported_markup_is_shown_as_text_and_never_run(page: Chrome, pristine_server: str) -> None:
    ...
    assert (
        page.evaluate(
            "document.querySelectorAll('#page-manage .cvr img, #page-manage .cvr script,"
            " #page-manage .cvr a').length"
        )
        == 0
    )
    cards = page.evaluate(
        "[...document.querySelectorAll('#page-manage .ev__proposal')].map(n => n.textContent)"
    )
